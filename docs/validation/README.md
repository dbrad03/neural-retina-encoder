...
- **Timing/utilization:** two routed reports are tracked. (1) Standalone datapath: `vivado/timing_report.txt` / `util_report.txt` are routed `neuron_array_controller` reports from Vivado 2025.1 — **WNS +0.449 ns, WHS +0.091 ns** at 100 MHz. (2) Full overlay: `vivado/system_timing_summary.txt` / `system_util_summary.txt` are the routed `system_wrapper` reports emitted by `build_bd.tcl` on each bitstream build — **WNS +0.378 ns, WHS +0.018 ns** at 100 MHz (this supersedes the earlier uncommitted +0.305 figure; the worst setup path is in the engine pipeline, `controller_inst/engine_inst/v_s3_reg[17]`). These are from the post-start-gating build; the gate adds a single combinational term to the start path and did not regress timing. The full-wrapper timing sweep (`timing_sweep_v2.tcl`, Pass B) characterizes the WNS distribution across implementation strategies.
