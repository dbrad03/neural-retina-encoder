...
- **Cocotb RTL regression:** `make verify` covers eleven tests and is the source for functional RTL verification claims. Latest local run: 2026-06-23, all eleven tests passed (including the 300-frame full system integration test, the DMA pixel-ingress AXI-Stream adapter, and the DMA-enabled wrapper integration test).
- **Fixed-point accuracy (scope):** `test_golden.py` checks the RTL **bit-exact against a fixed-point reference** that uses the same Q-format and operations — i.e. implementation *consistency*, not accuracy versus an independent model. Error versus a floating-point reference is characterized offline in `sim/fixed_point_compare.cpp` (not part of `make verify`, no automated threshold). Treat "matches float" claims accordingly.
...
- **Spike packetization (caveats):** the design emits one TLAST-delimited packet per frame **that produces ≥1 spike**; a zero-spike frame emits no beat and therefore no packet (software treats RDFO==0 as "no spikes this frame"). Software must finish draining a frame's packet before the next `start` — a new `start` clears the drain gate (cutting a still-in-flight packet's TLAST). The hardware now exposes a **drain-busy status bit (bit7)** for software to poll; it does **not** hard-reject a premature start (back-to-back frames are intentional), so software must wait for `!busy` before the next `start`.
- **AXI-Lite / DMA BRAM arbitration (caveat):** with `USE_DMA_INGRESS`, a DMA pixel beat has unconditional priority on the shared BRAM port. The two paths are not meant to be used concurrently (DMA replaces the per-pixel AXI-Lite writes); a simultaneous AXI-Lite access during an active DMA transfer can be silently overridden without a bus error. Do not issue AXI-Lite pixel writes/reads while a DMA transfer is in flight.
- **DMA stimulus-input path:** board-validated on Zybo Z7-20 (2026-06-23). The DMA-enabled overlay (`axi_dma_0` on PS HP0) loads, `dma_frame_loaded` (status bit3) asserts after the burst, and the retina computes identical output to the AXI-Lite path. Routed `system_wrapper` timing closes at 100 MHz with the DMA added (WNS +0.305 ns, WHS +0.033 ns) — **from a local `build_bd.tcl` run on 2026-06-23; the full-system routed report is not yet committed** (only the standalone-controller report is tracked, see below), so this figure is not independently reproducible from the repo alone. See the timing benchmark below.
- **Timing/utilization:** The checked-in `vivado/timing_report.txt` and `vivado/util_report.txt` are routed standalone `neuron_array_controller` reports from Vivado 2025.1, generated 2026-06-15. They are not full `system_wrapper` reports.
