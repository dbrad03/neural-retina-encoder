...
- **DMA stimulus-input path:** board-validated on Zybo Z7-20 (2026-06-23). The DMA-enabled overlay (`axi_dma_0` on PS HP0) loads, `dma_frame_loaded` (status bit3) asserts after the burst, and the retina computes identical output to the AXI-Lite path. Routed `system_wrapper` timing closes at 100 MHz with the DMA added (WNS +0.305 ns, WHS +0.033 ns). See the timing benchmark below.
